...
export default function SectionHeader({
    ...
    if (align === 'center') {
        return (
            <CornerFrame extraClass={`mb-6 md:mb-8 flex flex-col items-center px-8 py-6 text-center ${className}`}>
                {CodeTagBadge}
                {title && (
    ...
    if (align === 'between') {
        return (
            <CornerFrame extraClass={`mb-6 md:mb-8 flex flex-wrap items-end justify-between gap-8 px-8 py-6 max-[900px]:flex-col max-[900px]:items-start ${className}`}>
    ...
    // Default: 'left'
    return (
        <CornerFrame extraClass={`mb-6 md:mb-8 flex flex-col items-start px-8 py-6 ${className}`}>
            {CodeTagBadge}
            {title && (
                <HeadingTag ref={titleRef} className={`${titleSizeClasses} font-extrabold text-slate-900 dark:text-white ${animClass}`}>
                    {renderTitle()}
