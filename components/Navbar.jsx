...
export default function Navbar() {
    useEffect(() => {
        ...
        // Removed dynamic color toggling for Vercel ultra-dark theme

        const cleanups = [];
        const logoTruus = document.querySelector('.logo-truus');
        if (logoTruus && WIGGLE_CONFIG) cleanups.push(initWiggle(logoTruus, WIGGLE_CONFIG.logoTruus || 5));
    ...
    return (
        <>
            <div className="nav-overlay fixed inset-0 bg-black/60 backdrop-blur-sm z-[99] opacity-0 invisible pointer-events-none"></div>
            <nav className="navbar fixed top-0 left-0 w-full flex justify-between items-center px-5 md:px-10 py-4 bg-black/40 backdrop-blur-lg border-b border-white/5 z-[1000] transition-colors duration-500 ease-[cubic-bezier(0.4,0,0.2,1)] text-white">
                <div className="nav-left nav-hover-trigger relative group/navleft" style={{ cursor: "url('/assets/Cursor SVG/cursor-pointer.svg') 12 12, pointer" }}>
                    <div className="logo-work-container relative flex justify-center items-center w-[60px] h-[55px] z-[105]">
                        <img src="/assets/Navbar SVG/nav-work-blob.svg" width="60" height="55" className="nav-bar__work-blob-svg absolute top-0 left-0 w-full h-full z-[1] grayscale" alt="" aria-hidden="true" />
                        <span className="logo-work-text relative z-[2] font-epilogue font-extrabold text-2xl md:text-[2rem] ml-[80px] tracking-tight transition-colors duration-500 ease-[cubic-bezier(0.4,0,0.2,1)] group-hover/navleft:!text-brand-dark">Nexora</span>
                    </div>

                    <div className="nav-popout nav-work-box absolute top-2 -left-2 w-[320px] h-auto bg-brand-bg text-brand-dark rounded-2xl opacity-0 invisible pointer-events-none z-[100] overflow-hidden group-hover/navleft:pointer-events-auto">
                        <div className="nav-popout-inner flex flex-col w-full h-full pt-[70px] pb-6 px-6 gap-5">
                            <div className="nav-work-item flex items-start gap-4 no-underline text-inherit cursor-pointer">
                                <div className="nav-work-item__text flex flex-col items-start gap-1.5 mt-0.5">
                                    <span className="nav-work-badge badge-blue bg-brand-cyan text-brand-navy text-[1.05rem] font-bold px-3 py-1 rounded-[20px] rounded-bl-[1px] w-fit">Web</span>
                                    <h4 className="nav-work-title mt-[50px] text-[1.1rem] font-[650] leading-[1.1] tracking-tight font-epilogue m-0">Web App Development</h4>
                                </div>
                            </div>
                            <div className="nav-work-item flex items-start gap-4 no-underline text-inherit cursor-pointer">
                                <div className="nav-work-item__text flex flex-col items-start gap-1.5 mt-0.5">
                                    <span className="nav-work-badge badge-green bg-brand-blue text-white text-[1.05rem] font-bold px-3 py-1 rounded-[20px] rounded-bl-[1px] w-fit">Mobile</span>
                                    <h4 className="nav-work-title mt-[50px] text-[1.1rem] font-[650] leading-[1.1] tracking-tight font-epilogue m-0">Native Apps (iOS/Android)</h4>
                                </div>
                            </div>
                            <div className="nav-work-item flex items-start gap-4 no-underline text-inherit cursor-pointer">
                                <div className="nav-work-item__text flex flex-col items-start gap-1.5 mt-0.5">
                                    <span className="nav-work-badge badge-purple bg-brand-navy border border-brand-cyan/20 text-brand-cyan text-[1.05rem] font-bold px-3 py-1 rounded-[20px] rounded-bl-[1px] w-fit">UI/UX</span>
                                    <h4 className="nav-work-title mt-[50px] text-[1.1rem] font-[650] leading-[1.1] tracking-tight font-epilogue m-0 text-white">Premium Design Systems</h4>
                                </div>
                            </div>
                            <a href="#services" className="nav-work-btn block text-center bg-brand-cyan text-black p-5 rounded-xl font-extrabold text-[1.4rem] no-underline mt-[10rem] cursor-pointer font-epilogue transition-transform duration-300 hover:scale-105">
                                <span className="nav-work-btn__text inline-block">View All Services</span>
                            </a>
                        </div>
                    </div>
                </div>
                ...
                <div className="nav-center" style={{ cursor: "url('/assets/Cursor SVG/cursor-pointer.svg') 12 12, pointer" }}>
                    <div className="text-3xl md:text-4xl font-black font-epilogue tracking-tight flex items-center gap-1 group">
                        <span>N E</span>
                        <span className="text-brand-cyan group-hover:scale-110 transition-transform duration-300">X</span>
                        <span>O R A</span>
                    </div>
                </div>
                
                <div className="nav-right nav-hover-trigger relative group/navright" style={{ cursor: "url('/assets/Cursor SVG/cursor-pointer.svg') 12 12, pointer" }}>
                    <a href="https://wa.me/1234567890" target="_blank" rel="noopener noreferrer" className="logo-whatsapp relative z-[105]">
                        <svg xmlns="http://www.w3.org/2000/svg" width="32" height="32" viewBox="0 0 25 27" fill="none" className="nav-bar__whatsapp-svg w-8 h-auto block text-[#51cd5f] transition-colors duration-200 ease-[cubic-bezier(0.4,0,0.2,1)]">
